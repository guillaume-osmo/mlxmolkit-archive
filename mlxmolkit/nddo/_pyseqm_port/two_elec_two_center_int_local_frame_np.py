...
#     riHH = ev/sqrt(r0[HH]**2 + exp_mask * (rho0a[HH]+rho0b[HH])**2)
    ...
    coreHH = torch.zeros(HH.sum(),2,dtype=dtype)
    #ni=nj=1
    ...
    riXH = torch.zeros(XH.sum(),4,dtype=dtype)
    ...
    coreXH = torch.zeros(XH.sum(),5,dtype=dtype)
    ...
    ri =  torch.zeros(XX.sum(),22,dtype=dtype)
    core = torch.zeros(XX.sum(),8,dtype=dtype)
