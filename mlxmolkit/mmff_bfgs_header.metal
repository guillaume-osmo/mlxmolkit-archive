...
// ---- Gather-based gradient: returns dE/d(pos[my_atom*3+my_coord]) for one term ----
// These avoid scatter-add, enabling parallel gradient computation.

inline float bond_stretch_g_gather(
    const device float* pos, int i1, int i2, float kb, float r0,
    int my_atom, int my_coord
) {
    if (my_atom != i1 && my_atom != i2) return 0.0f;
    float dx = pos[i1*3+0] - pos[i2*3+0];
    float dy = pos[i1*3+1] - pos[i2*3+1];
    float dz = pos[i1*3+2] - pos[i2*3+2];
    float d2 = dx*dx + dy*dy + dz*dz;
    ...
    float de_dr = 143.9325f * kb * dr * (1.0f - 3.0f * dr + (14.0f/3.0f) * dr * dr);
    float f = de_dr * inv_d;
    float dif = (my_coord == 0) ? dx : (my_coord == 1) ? dy : dz;
    return (my_atom == i1) ? f * dif : -f * dif;
}

inline float angle_bend_g_gather(
    const device float* pos, int i1, int i2, int i3,
    float ka, float theta0, bool is_linear,
    int my_atom, int my_coord
) {
    if (my_atom != i1 && my_atom != i2 && my_atom != i3) return 0.0f;
    AngleData ad = angle_internals(pos, i1, i2, i3);
    float sinTsq = 1.0f - ad.cosT * ad.cosT;
    if (sinTsq < 1e-16f) return 0.0f;
    float inv_neg_sinT = -rsqrt(max(sinTsq, 1e-16f));
    float dtheta = ad.theta_deg - theta0;
    float de;
    if (is_linear) {
    ...
    float g1 = cf * inter1;
    float g3 = cf * inter3;
    if (my_atom == i1) return g1;
    if (my_atom == i3) return g3;
    return -(g1 + g3); // i2
}

inline float stretch_bend_g_gather(
    const device float* pos, int i1, int i2, int i3,
    float r0_ij, float r0_kj, float theta0, float kba_ij, float kba_kj,
    int my_atom, int my_coord
) {
    ...
    float beis = RAD_TO_DEG * (kba_ij*dr_ij + kba_kj*dr_kj) * invSinT;
    float r1h[3], r2h[3];
    for (int dd = 0; dd < 3; dd++) { r1h[dd] = ad.r1[dd]*ad.inv_d1; r2h[dd] = ad.r2[dd]*ad.inv_d2; }
    int dd = my_coord;
    float inter1 = ad.inv_d1 * (r2h[dd] - ad.cosT*r1h[dd]);
    float inter3 = ad.inv_d2 * (r1h[dd] - ad.cosT*r2h[dd]);
    if (my_atom == i1) return pf * (dtheta*r1h[dd]*kba_ij - inter1*beis);
    if (my_atom == i3) return pf * (dtheta*r2h[dd]*kba_kj - inter3*beis);
    return pf * (-dtheta*(r1h[dd]*kba_ij+r2h[dd]*kba_kj) + (inter1+inter3)*beis); // i2
}

inline float vdw_g_gather(
    const device float* pos, int i1, int i2, float R_star, float epsilon,
    int my_atom, int my_coord
) {
    if (my_atom != i1 && my_atom != i2) return 0.0f;
...
inline float oop_bend_g_gather(
    const device float* pos, int i1, int i2, int i3, int i4, float koop,
    int my_atom, int my_coord
) {
    if (my_atom != i1 && my_atom != i2 && my_atom != i3 && my_atom != i4) return 0.0f;
    float rJI[3], rJK[3], rJL[3];
    for (int d = 0; d < 3; d++) {
        rJI[d] = pos[i1*3+d] - pos[i2*3+d];
        rJK[d] = pos[i3*3+d] - pos[i2*3+d];
    ...
    float term1 = invCosChi * invSinTheta;
    float term2 = sinChi * invCosChi * invSinTheta * invSinTheta;
    float t1[3], t2t[3], t3[3];
    t1[0]=dJL[1]*dJK[2]-dJL[2]*dJK[1]; t1[1]=dJL[2]*dJK[0]-dJL[0]*dJK[2]; t1[2]=dJL[0]*dJK[1]-dJL[1]*dJK[0];
    t2t[0]=dJI[1]*dJL[2]-dJI[2]*dJL[1]; t2t[1]=dJI[2]*dJL[0]-dJI[0]*dJL[2]; t2t[2]=dJI[0]*dJL[1]-dJI[1]*dJL[0];
    t3[0]=dJK[1]*dJI[2]-dJK[2]*dJI[1]; t3[1]=dJK[2]*dJI[0]-dJK[0]*dJI[2]; t3[2]=dJK[0]*dJI[1]-dJK[1]*dJI[0];
    int d = my_coord;
    float tg1 = (t1[d]*term1 - (dJI[d]-dJK[d]*cosTheta)*term2) * idJI;
    float tg3 = (t2t[d]*term1 - (dJK[d]-dJI[d]*cosTheta)*term2) * idJK;
    float tg4 = (t3[d]*term1 - dJL[d]*sinChi*invCosChi) * idJL;
    float g1 = dE_dChi * tg1;
    float g3 = dE_dChi * tg3;
    float g4 = dE_dChi * tg4;
    if (my_atom == i1) return g1;
    if (my_atom == i3) return g3;
    if (my_atom == i4) return g4;
    return -(g1 + g3 + g4); // i2
}

inline float torsion_g_gather(
    const device float* pos, int i1, int i2, int i3, int i4,
    float V1, float V2, float V3,
    int my_atom, int my_coord
) {
    ...
    float n1sq = c1[0]*c1[0]+c1[1]*c1[1]+c1[2]*c1[2];
    float n2sq = c2[0]*c2[0]+c2[1]*c2[1]+c2[2]*c2[2];
    if (n1sq < 1e-30f || n2sq < 1e-30f) return 0.0f;
    float in1 = min(rsqrt(n1sq), 1e5f);
    float in2 = min(rsqrt(n2sq), 1e5f);
    for (int d = 0; d < 3; d++) { c1[d]*=in1; c2[d]*=in2; }
    float cosPhi = clamp(c1[0]*c2[0]+c1[1]*c2[1]+c1[2]*c2[2], -1.0f, 1.0f);
    ...
    int d = my_coord;
    if (my_atom == i1) {
        float cr[3]; cr[0]=dT0[2]*dx2[1]-dT0[1]*dx2[2]; cr[1]=dT0[0]*dx2[2]-dT0[2]*dx2[0]; cr[2]=dT0[1]*dx2[0]-dT0[0]*dx2[1];
        return sinTerm * cr[d];
    }
    if (my_atom == i4) {
        float cr[3]; cr[0]=dT1[1]*(-dx2[2])-dT1[2]*(-dx2[1]); cr[1]=dT1[2]*(-dx2[0])-dT1[0]*(-dx2[2]); cr[2]=dT1[0]*(-dx2[1])-dT1[1]*(-dx2[0]);
        return sinTerm * cr[d];
    }
    if (my_atom == i2) {
        float g2[3];
        g2[0] = sinTerm*(dT0[1]*(dx2[2]-dx1[2])+dT0[2]*(dx1[1]-dx2[1])+dT1[1]*(-dx4[2])+dT1[2]*dx4[1]);
        g2[1] = sinTerm*(dT0[0]*(dx1[2]-dx2[2])+dT0[2]*(dx2[0]-dx1[0])+dT1[0]*dx4[2]+dT1[2]*(-dx4[0]));
        g2[2] = sinTerm*(dT0[0]*(dx2[1]-dx1[1])+dT0[1]*(dx1[0]-dx2[0])+dT1[0]*(-dx4[1])+dT1[1]*dx4[0]);
        return g2[d];
    }
    // i3
    float g3[3];
    g3[0] = sinTerm*(dT0[1]*dx1[2]+dT0[2]*(-dx1[1])+dT1[1]*(dx4[2]+dx2[2])+dT1[2]*(-dx4[1]-dx2[1]));
    g3[1] = sinTerm*(dT0[0]*(-dx1[2])+dT0[2]*dx1[0]+dT1[0]*(-dx4[2]-dx2[2])+dT1[2]*(dx4[0]+dx2[0]));
    g3[2] = sinTerm*(dT0[0]*dx1[1]+dT0[1]*(-dx1[0])+dT1[0]*(dx4[1]+dx2[1])+dT1[1]*(-dx4[0]-dx2[0]));
    return g3[d];
}
