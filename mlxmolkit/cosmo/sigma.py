...
def average_sigma(
    ...
    # Squared distances between all segment pairs
    dist_sq = cdist(seg_pos, seg_pos, 'sqeuclidean')

    # w(i,j), transposed so that row i holds the weights acting on segment i
    exp_arr = np.exp(-dist_sq * inv_rad[:, np.newaxis])
    weight = (r_av_sq * inv_rad[:, np.newaxis] * exp_arr).T

    # Area-weighted average (A_j ∝ r_seg_j²)
...
def compute_sigma_profile(
    ...
    # Fractional bin index for each segment
    idx_float = (seg_sigma - sigma_grid[0]) / SIGMA_GRID_STEP
    idx = np.floor(idx_float).astype(np.int64)
    frac = idx_float - idx

    # Keep the linear-interpolation stencil (idx, idx+1) inside the grid
    idx = np.clip(idx, 0, n_bins - 2)
    frac = np.clip(frac, 0.0, 1.0)

    # Linear interpolation onto the two straddling bins
...
def classify_segments(
    ...
    # Donor: H with negative sigma (positive partial charge)
    is_H = np.isin(seg_element, list(HB_DONOR_ELEMENTS))
    seg_hb_type[is_H & (seg_sigma < 0)] = 1

    # Acceptor: N/O/F/S with positive sigma (negative partial charge)
...
def full_sigma_analysis(cosmo_result: dict, atoms: list[int]) -> dict:
    ...
    # Gaussian averaging over the local surface region
    ...
    sigma_moment_1 = np.sum(sigma_grid * p_norm) * total_area
    sigma_moment_2 = np.sum(sigma_grid ** 2 * p_norm) * total_area
